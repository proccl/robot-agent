classdef Arm7R < handle
    ...
    methods
        ...
        function [t, err] = inverseKinematics(obj, T)
            ...
            % 驗證 t2 是否存在實數解：此判斷驗證解析閉式解的數學存在性
            % 若觸發，通常是輸入位姿 T 的浮點精度不足導致（FK 累積誤差），
            % 而非位姿本身在工作空間外。不應加入容差，以免掩蓋真正的無解情況。
        ...
        %    t2 = obj.angleRound(t2, 2);
           ...
           % t7 = obj.angleRound(t7, 7);
            ...
            % 驗證 t34 是否存在實數解：同樣是解析閉式解的數學存在性判斷
            % 浮點誤差可能導致假性無解，應從 FK 精度或腳本端處理，而非修改此處。
            if (1 - (E/rou2)^2) < 0
                err = 1;
                t = 0;
                return;
